...
import {
  fetchGroups,
  fetchKnowledgeChunkAccessGrants,
  fetchKnowledgeCollectionAccessGrants,
  fetchKnowledgeItemAccessGrants,
  fetchKnowledgeSourceAccessGrants,
  loadSession,
  revokeKnowledgeChunkShare,
  revokeKnowledgeCollectionShare,
  revokeKnowledgeItemShare,
...
type WorkspaceKnowledgeAccessModalProps = {
  mode: 'item' | 'collection' | 'source' | 'chunk'
  open: boolean
  onClose: () => void
  resourceId: string
...
export function WorkspaceKnowledgeAccessModal({
  mode,
  open,
  onClose,
  resourceId,
  resourceLabel
}: WorkspaceKnowledgeAccessModalProps) {
  ...
  async function loadData(): Promise<void> {
    ...
    try {
      const [grantsResponse, groupsResponse] = await Promise.all([
        mode === 'item'
          ? fetchKnowledgeItemAccessGrants(resourceId, session)
          : mode === 'collection'
            ? fetchKnowledgeCollectionAccessGrants(resourceId, session)
            : mode === 'source'
              ? fetchKnowledgeSourceAccessGrants(resourceId, session)
              : fetchKnowledgeChunkAccessGrants(resourceId, session),
  ...
  async function handleShare(): Promise<void> {
    ...
    try {
      ...
      if (mode === 'item') {
        await shareKnowledgeItem(resourceId, input, session)
      } else if (mode === 'collection') {
        await shareKnowledgeCollection(resourceId, input, session)
      } else if (mode === 'source') {
        await shareKnowledgeSource(resourceId, input, session)
      } else {
        await shareKnowledgeChunk(resourceId, input, session)
      }
  ...
  async function handleRevoke(grantId: string): Promise<void> {
    ...
    try {
      if (mode === 'item') {
        await revokeKnowledgeItemShare(resourceId, grantId, session)
      } else if (mode === 'collection') {
        await revokeKnowledgeCollectionShare(resourceId, grantId, session)
      } else if (mode === 'source') {
        await revokeKnowledgeSourceShare(resourceId, grantId, session)
      } else {
        await revokeKnowledgeChunkShare(resourceId, grantId, session)
      }
