...
export function WorkspaceKnowledge({ initialView: _initialView = 'documents' }: WorkspaceKnowledgeProps) {
  ...
  const [knowledgeView] = useState<'documents' | 'collections' | 'sources' | 'chunks'>('documents')
  ...
  const [visibilityFilter, setVisibilityFilter] = useState<'all' | 'shared'>('all')
  ...
  useEffect(() => {
    setPage(1)
  }, [visibilityFilter, query, pageSize, documentSortBy, sourceSortBy, chunkSortBy, sortDirection])
    ...
    <div className="workspace-resource-page">
      ...
      >
        <form id="quick-knowledge-text-form" className="workspace-editor-form" onSubmit={(event) => void handleQuickTextSubmit(event)}>
          <Field label="Name">
            <FieldInput onChange={(event) => setQuickTextName(event.target.value)} placeholder="Runbook" value={quickTextName} />
          </Field>
          <Field label="Collection">
            <FieldSelect onChange={(event) => setQuickTextCollectionId(event.target.value)} value={quickTextCollectionId}>
      ...
      >
        <form id="quick-knowledge-web-form" className="workspace-editor-form" onSubmit={(event) => void handleQuickWebSubmit(event)}>
          ...
          <Field label="Name">
            <FieldInput onChange={(event) => setQuickWebName(event.target.value)} placeholder="Product doc page" value={quickWebName} />
          </Field>
          <Field label="Collection">
            <FieldSelect onChange={(event) => setQuickWebCollectionId(event.target.value)} value={quickWebCollectionId}>
              <option value="collection_general">General</option>
              {collections
                .filter((collection) => collection.id !== 'collection_general')
                .map((collection) => (
      ...
      >
        <div className="workspace-editor-form">
          <Field label="Mode">
            <FieldSelect onChange={(event) => setDirectoryMode(event.target.value as 'upload' | 'sync')} value={directoryMode}>
              <option value="upload">Upload (append)</option>
              <option value="sync">Sync (replace collection documents)</option>
            </FieldSelect>
          </Field>
          <Field label="Collection">
            <FieldSelect onChange={(event) => setDirectoryCollectionId(event.target.value)} value={directoryCollectionId}>
              <option value="collection_general">General</option>
              {collections
                .filter((collection) => collection.id !== 'collection_general')
                .map((collection) => (
      ...
      >
        <div className="workspace-editor-form">
          <Field label="Collection">
            <FieldSelect onChange={(event) => setMoveCollectionId(event.target.value)} value={moveCollectionId}>
              <option value="collection_general">General</option>
              {collections
                .filter((collection) => collection.id !== 'collection_general')
                .map((collection) => (
      ...
      />
      <PageHeader
        actions={canManage ? <CapsuleButton href="/workspace/knowledge/create" variant="primary">New knowledge</CapsuleButton> : null}
        description="Create and manage reusable knowledge bases."
        label="Workspace"
        title="Knowledge"
      />
      ...
      <section className="workspace-resource-section">
        <div className="workspace-resource-filters">
          ...
          <Field label="View">
            <FieldSelect onChange={(event) => setVisibilityFilter(event.target.value as 'all' | 'shared')} value={visibilityFilter}>
        ...
        {!loading && sortedDocuments.length === 0 ? <WorkspaceEmptyState title="No knowledge found." /> : null}
        {!loading ? (
